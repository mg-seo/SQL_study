...
CREATE TABLE DEPTS(
    ...
    DEPT_DATE DATE,
    DEPT_BONUS NUMBER(10, 2), -- 정수 10자리, 소수점 2자리수까지 저장
    DEPT_CONTENT LONG -- 2기가 가변문자열 (VARCHAR2 보다 더 큰)
);
DESC DEPTS;
SELECT * FROM DEPTS;
...
----------------------------------------------------------------------------
-- 테이블 삭제
DROP TABLE DEPTS CASCADE; --테이블이 가지는 FK제약조건을 삭제하면서, 테이블을 날려버림(위험)
DROP TABLE DEPARTMENTS; -- DEPARTMENTS 는 EMPLOYEES 테이블과 참조관계를 가지고 있어 한 번에 삭제되지 않음